...
class ClassBuilder
{
	 ...
	 */
	public static function mapDB(IConnectable $conn, bool $prettyNames = true, string $classPath = null): void
	{
	 ...
	 */
	public static function mapTables($tables, bool $prettyNames = true, string $classPath = null): void
	{
		...
		foreach ($tables as $table => $fields) {
			...
			foreach ($fields as $field => $info) {
				...
				$class_code .= "\t//$field {$info['type']}" . (!$info['nullable'] ? ' NOT NULL' : '') . PHP_EOL;
		...
		foreach($idxs as $idx) {
			...
			if(is_numeric($rul[$idx])) {
				if($ret[0] === 'int' || $ret[0] === 'string') {
					$ret[1] .= 'strlen(' . ($ret[0] === 'int' ? '(string)' : '') . "$ret[2])" . (strpos($type, 'char') === 0 ? ' === ' : ' <= ') . $rul[$idx];
				} elseif($ret[0] === 'float' && preg_match('/\d*,\d*/', $rul[$idx])) {
					list($int, $dec) = explode(',', $rul[$idx]);
					$ret[1] .= "strlen((string)$ret[2]) <= $int && strlen((string)((int)$ret[2] - floor($ret[2]))) <= $dec";
				}
			} elseif($rul[$idx] === 'unsigned') {
				$ret[1] .= $ret[2]  . ' > 0';
			} elseif(preg_match('/\w*(,\s?\w*)?/', $rul[$idx])) {
