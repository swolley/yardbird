# php-db-extended
**php-db-extended** is a wrapper for multiple types of databases (currently supported are all PDO drivers, Mysqli, OCI8, MongoDB). The library exposes common methods for crud functions and a parser class to translate sql queries to mongodb library syntax.
The project is still in progress and not totally tested.

## requirements
**php-db-extended** requires php mongodb driver and mongodb/mongodb library if you want to connect to MongoDB.
...
$connection = (new DBFactory)($options);
```
### crud methods
```php
/**
...
*/
$connection->select($table, $columns = [], $where = [], $join = [], $orderBy = [], $limit = null, $fetchMode = DBFactory::FETCH_ASSOC, $fetchModeParam = 0, $fetchPropsLateParams = []);
...
*/
$connection->procedure($name, $inParams = [], $outParams = [], $fetchMode = DBFactory::FETCH_ASSOC, $fetchModeParam = 0, $fetchPropsLateParams = []);
